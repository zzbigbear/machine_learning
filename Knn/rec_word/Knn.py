...
def handwritingClassTest():
    ...
    for i in range(mTest):
        ...
        fileStr = fileNameStr.split('.')[0]
        classNumStr = int(fileStr.split('_')[0])
        vecterUndertest = img2vector('testDigits/%s'%fileNameStr)
        classifyresult = classify0(vecterUndertest,trainingMat,hwLabels,3)
    ...
    print("the total error %d"%errorCount)
    print("total error rate %f"%(errorCount/float(mTest)))
